...
def inspect_file(file_path):
    ...
    if file_path.endswith('.xlsx'):
        # Calamine engine is fast for Excel
...
def query_data(parquet_path, query_json):
    ...
    query_json format: {"group_by": ["col1"], "agg": {"col2": "sum"}}
    """
    ...
    for col, func in aggs.items():
        if func == "sum":
            ...
            agg_list.append(pl.col(col).mean())
        elif func == "count":
            agg_list.append(pl.col(col).count())
